...
function Note({ id, date, time, name, procedure, phoneNumber, additionalNotes, status, onStatusChange, onDelete }) {
  ...
  const formatedDate = formatter.format(noteDate);
  ...
  return (
    <div className={styles.note}>
      <div className={styles.top}>
        <p className={styles.time}>{`${formatedDate}, ${time}`}</p>
      ...
      <select className={styles.status} value={status.toLowerCase()} onChange={handleStatusChange} aria-label="Статус запису">
